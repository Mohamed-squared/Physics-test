...
function loadData() {
    ...
    if (!data) {
        ...
        let num_chapters = parseInt(prompt("How many chapters have you studied?"));
        if (isNaN(num_chapters) || num_chapters < 1) num_chapters = 1;
        for (let i = 1; i <= num_chapters; i++) {
            let total_questions = parseInt(prompt(`Enter total questions for chapter ${i}:`));
            if (isNaN(total_questions) || total_questions < 1) total_questions = 1;
            let chap_num = i.toString();
            subject.chapters[chap_num] = {
                total_questions,
                total_attempted: 0,
                total_wrong: 0,
            ...
            if (usedQuestions[chap_num]) {
                subject.chapters[chap_num].available_questions = subject.chapters[chap_num].available_questions.filter(q => !usedQuestions[chap_num].includes(q));
            }
        }
        saveData(data);
